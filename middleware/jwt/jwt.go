...
// This is a customized version of Echo JWT middleware for Logit
...
//
// See: https://jwt_go.io/introduction
...
func JWTWithConfig(config JWTConfig) echo.MiddlewareFunc {
	...
	config.keyFunc = func(t *jwt_go.Token) (interface{}, error) {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			token := new(jwt_go.Token)
			// Issue #647, #656
			if _, ok := config.Claims.(jwt_go.MapClaims); ok {
				token, err = jwt_go.Parse(auth, config.keyFunc)
			} else {
